...
serve(async (req) => {
  try {
    ...
    if (!documentUrl) {
      return new Response(
        JSON.stringify({ error: 'Missing documentUrl' }),
        { headers: { 'Content-Type': 'application/json' }, status: 400 }
      );
    }

    // Pipeline Step 1: Document OCR/Extraction (Mocked)
    console.log(\`Extracting text from \${documentUrl} via Azure Document Intelligence...\`);
    const extractedText = "Mock extracted requirements: 1. SSO Integration. 2. 99.9% Uptime SLA.";
