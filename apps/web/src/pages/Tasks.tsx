...
export default function TasksPage() {
  ...
  const columns = [
    { id: 'Not Started', label: 'Not Started', matchStatuses: ['Not Started'], color: 'text-gray-500' },
    { id: 'In Progress', label: 'In Progress', matchStatuses: ['In Progress'], color: 'text-blue-600' },
    { id: 'Bottlenecks', label: 'Bottlenecks', matchStatuses: ['Waiting Customer', 'Waiting Internal', 'Blocked'], color: 'text-orange-600' },
    { id: 'Done', label: 'Done', matchStatuses: ['Completed'], color: 'text-green-600' }
  ];
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-8rem)] flex flex-col">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-primary tracking-tight">Tasks</h1>
          <p className="text-sm text-gray-500 mt-1">Manage your pre-sales tasks and follow-ups.</p>
        </div>
        <button 
      ...
      {loading ? (
        ...
        <div className="p-8 text-center text-danger bg-red-50 rounded-lg border border-red-100">{error}</div>
      ) : (
        <div className="flex gap-6 overflow-x-auto pb-4 items-start flex-1">
          {columns.map(col => {
            const colTasks = tasks.filter(t => col.matchStatuses.includes(t.status));
            return (
              <div key={col.id} className="flex-none w-[320px] bg-gray-50/50 rounded-xl flex flex-col max-h-full border border-gray-100">
                <div className="flex justify-between items-center px-4 py-3 border-b border-gray-100/80">
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${col.color.replace('text-', 'bg-')}`}></div>
                ...
                <div className="p-3 flex flex-col gap-3 overflow-y-auto">
                  {colTasks.length === 0 ? (
                    <div className="text-xs text-gray-400 text-center py-6 border-2 border-dashed border-gray-200 rounded-lg">No tasks</div>
                  ) : (
                    colTasks.map(task => (
                      <div 
                        key={task.id}
