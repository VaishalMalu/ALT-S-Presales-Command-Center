...
export default function DashboardPage() {
  ...
  useEffect(() => {
    Promise.all([getOpportunities(), getAccounts(), getTasks()]).then(([opps, accs, tsks]) => {
  ...
  const stats = useMemo(() => {
    const weightedPipeline = opportunities
      ...
      .reduce((s, o) => s + (Number(o.deal_value) || 0) * (Number(o.probability) || 0), 0);
    ...
    opportunities.forEach(o => {
      if (o.stage !== 'Closed Won' && o.stage !== 'Closed Lost' && checkOverdue(o.deadline)) overdue++;
    });
    tasks.forEach(t => {
      if (t.status !== 'Completed' && checkOverdue(t.due_date)) overdue++;
    });
    accounts.forEach(a => {
      if (a.health_score !== null && a.health_score < 80 && checkOverdue(a.renewal_date)) overdue++;
    });
  ...
  const upcoming = useMemo(() => {
    ...
    opportunities.forEach(o => {
      if (o.stage !== 'Closed Won' && o.stage !== 'Closed Lost') {
        ...
        if (d !== null && d >= 0 && d <= 14) list.push({ label: `${o.title} — RFP deadline`, days: d });
      }
    });
    tasks.forEach(t => {
      if (t.status !== 'Completed') {
        const d = getDays(t.due_date);
        if (d !== null && d >= 0 && d <= 14) list.push({ label: `${t.title} — Task due`, days: d });
      }
    });
    accounts.forEach(a => {
      const d = getDays(a.renewal_date);
      if (d !== null && d >= 0 && d <= 14) list.push({ label: `${a.name} — Renewal`, days: d });
    });
  ...
  const stageLabels = ["Lead Created", "Customer Qualification", "Solution Design", "Proposal/RFP Submitted", "Closed Won", "Closed Lost"];
  const stageCounts = stageLabels.map(label => opportunities.filter(o => o.stage === label).length);
  ...
  const healthCounts = [
    ...
    accounts.filter(a => (a.health_score || 0) >= 50 && (a.health_score || 0) < 80).length,
  ...
  opportunities.forEach(o => {
    if (o.stage !== 'Closed Won' && o.stage !== 'Closed Lost') {
      ...
      entityTotals[key] = (entityTotals[key] || 0) + (Number(o.deal_value) || 0) * (Number(o.probability) || 0);
    }
  });
  const entityLabels = Object.keys(entityTotals);
  const entityData = entityLabels.map(k => entityTotals[k]);
  
  const cleanPalette = ["#3B82F6", "#8B5CF6", "#14B8A6", "#F59E0B", "#6366F1", "#EC4899", "#64748B"];
  ...
  const doughnutOptions = {
    ...
    plugins: {
      legend: { 
        ...
        labels: { padding: 20, usePointStyle: true, pointStyle: 'circle' as const, font: { size: 12 } } 
      },
      tooltip: commonTooltip,
    }
  };
    ...
    <div className="space-y-6 pb-12 bg-gray-50 min-h-screen -m-6 p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Overview of your pipeline and active operations.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Weighted Pipeline', value: stats.weightedPipeline, isRisk: false },
          { label: 'Total ARR', value: stats.totalArr, isRisk: false },
          { label: 'Open Tasks', value: stats.openTasks, isRisk: false },
          { label: 'Overdue Items', value: stats.overdue, isRisk: Number(stats.overdue) > 0 },
        ].map((kpi, i) => (
          <div key={i} className={`bg-white rounded-lg shadow-sm border ${kpi.isRisk ? 'border-red-200' : 'border-gray-200'} p-5 flex flex-col justify-between`}>
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">{kpi.label}</span>
            <div className="mt-2 flex items-baseline justify-between">
              <span className={`text-2xl font-bold ${kpi.isRisk ? 'text-red-600' : 'text-gray-900'}`}>{kpi.value}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 flex flex-col min-h-[340px]">
          <h3 className="text-sm font-semibold mb-6 text-gray-800">Pipeline by Stage</h3>
        ...
        <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 flex flex-col min-h-[340px]">
          <h3 className="text-sm font-semibold mb-6 text-gray-800">Accounts by Health</h3>
          <div className="flex-1 min-h-[240px] relative">
        ...
        <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 flex flex-col min-h-[340px]">
          <h3 className="text-sm font-semibold mb-6 text-gray-800">Pipeline by Entity</h3>
          <div className="flex-1 min-h-[240px] relative">
      ...
      <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 mt-6">
        <h3 className="text-sm font-semibold mb-4 text-gray-800">Upcoming in Next 14 Days</h3>
        <div className="space-y-1">
          {upcoming.length === 0 ? (
            <div className="text-sm text-gray-500 py-4 text-center">No upcoming items.</div>
          ) : (
            upcoming.map((u, i) => (
              <div key={i} className="flex justify-between items-center py-2.5 border-b border-gray-100 last:border-0 hover:bg-gray-50 px-2 rounded-md transition-colors">
                <span className="text-gray-700 text-sm">{u.label}</span>
                <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                  u.days <= 7 ? 'bg-red-50 text-red-700' : 'bg-gray-100 text-gray-700'
                }`}>
